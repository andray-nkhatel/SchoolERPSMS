...
{
    public class MarkSchedulePdfService
    {
        private readonly SchoolDbContext _context;
        private readonly ILogger<MarkSchedulePdfService> _logger;

        public MarkSchedulePdfService(SchoolDbContext context, ILogger<MarkSchedulePdfService> logger)
        {
            _context = context;
            _logger = logger;
        ...
        private List<(Student Student, List<decimal> SubjectScores, List<int> SubjectPoints, decimal Total, decimal TotalPoints, List<bool> SubjectAbsentFlags)> GetMarkScheduleRows(
        ...
        {
            var rows = new List<(Student, List<decimal>, List<int>, decimal, decimal, List<bool>)>();
            foreach (var student in students)
            {
                var subjectScores = new List<decimal>();
                var subjectPoints = new List<int>();
                var subjectAbsentFlags = new List<bool>();
                ...
                foreach (var subject in subjects)
                {
                    ...
                    var isAbsent = score?.IsAbsent ?? false;
                    
                    subjectScores.Add(scoreValue);
                    subjectAbsentFlags.Add(isAbsent);
                ...
                rows.Add((student, subjectScores, subjectPoints, total, totalPoints, subjectAbsentFlags));
            }
            return rows;
        }
        ...
        // Helper method to calculate class average excluding absent students and zero scores
        private static decimal CalculateClassAverage(IEnumerable<decimal> scores, IEnumerable<bool> absentFlags)
        {
            var validScores = scores
                .Zip(absentFlags, (score, isAbsent) => new { Score = score, IsAbsent = isAbsent })
                .Where(x => !x.IsAbsent && x.Score > 0)
                .Select(x => x.Score)
                .ToList();
        ...
        // Helper method to calculate class average for a specific subject index
        private static decimal CalculateSubjectClassAverage(List<(Student Student, List<decimal> SubjectScores, List<int> SubjectPoints, decimal Total, decimal TotalPoints, List<bool> SubjectAbsentFlags)> rankedRows, int subjectIndex)
        {
            var validScores = rankedRows
                .Where(row => !row.SubjectAbsentFlags[subjectIndex] && row.SubjectScores[subjectIndex] > 0)
                .Select(row => row.SubjectScores[subjectIndex])
                .ToList();
            
            return validScores.Any() ? validScores.Average() : 0;
        }

        public async Task<byte[]> GenerateMarkSchedulePdfAsync(int academicYearId, int term, string examTypeName)
        {
           ...
           // _logger.LogInformation($"Loaded {grades.Count} grades from the database.");
            ...
            Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    // Add footer
                    page.Footer().AlignCenter().Text($"Term: {term}, Year: {academicYear.Name}").FontSize(8);
                    
                    page.Content().Column(col =>
                    {
                        col.Item().Text($"Mark Schedule - Academic Year {academicYearId}, Term {term}, Exam: {examTypeName}").FontSize(16).Bold().AlignCenter();

                        bool isFirstGrade = true;
                        foreach (var grade in grades)
                        {
                           // _logger.LogInformation($"Processing grade: {grade.FullName} (ID: {grade.Id})");
                            ...
                            var studentRows = GetMarkScheduleRows(students, subjects, scores, examTypeName, grade.Section);
                            ...
                            var rankedRows = studentRows
                                ...
                                .Select((r, idx) => new {
                                    ...
                                    SubjectAbsentFlags = r.SubjectAbsentFlags,
                                    Position = idx + 1
                                }).ToList();
                            ...
                            col.Item().Table(table =>
                            {
                                ...
                                table.Header(header =>
                                {
                                    ...
                                    foreach (var subject in subjects)
                                        header.Cell().Border(1).Background(Colors.Grey.Lighten3).Text(subject.Name).Bold();
                                ...
                                foreach (var row in rankedRows)
                                {
                                    ...
                                    {
                                        // For other sections: show only scores
                                ...
                                if (rankedRows.Count > 0)
                                {
                                    ...
                                    // Calculate averages for each subject (excluding absent students and zero scores)
                                    for (int i = 0; i < subjects.Count; i++)
                                    {
                                        var validScores = rankedRows
                                            .Where(row => !row.SubjectAbsentFlags[i] && row.SubjectScores[i] > 0)
                                            .Select(row => row.SubjectScores[i])
                                            .ToList();
                                        
                                        var average = validScores.Any() ? validScores.Average() : 0;
                                        
                                        if (isSeniorSecondary)
                                        {
                                            var validPoints = rankedRows
                                                .Where(row => !row.SubjectAbsentFlags[i] && row.SubjectScores[i] > 0)
                                                .Select(row => row.SubjectPoints[i])
                                                .ToList();
                                            var averagePoints = validPoints.Any() ? validPoints.Average() : 0;
                                    ...
                                    // Average total (excluding students who are absent for all subjects)
                                    var validTotalAverages = rankedRows
                                        .Where(row => !row.SubjectAbsentFlags.All(absent => absent) && row.Total > 0)
                                        .Select(row => row.Total)
                                        .ToList();
                                    var averageTotal = validTotalAverages.Any() ? validTotalAverages.Average() : 0;
                                    table.Cell().Border(1).Background(Colors.Blue.Lighten4).AlignCenter().Text(averageTotal.ToString("F1")).Bold();
                                    
                                    if (isSeniorSecondary)
                                    {
                                        var validTotalPointsAverages = rankedRows
                                            .Where(row => !row.SubjectAbsentFlags.All(absent => absent) && row.TotalPoints > 0)
                                            .Select(row => row.TotalPoints)
                                            .ToList();
                                        var averageTotalPoints = validTotalPointsAverages.Any() ? validTotalPointsAverages.Average() : 0;
        ...
        public async Task<byte[]> GenerateMarkSchedulePdfForGradeAsync(int gradeId, int academicYearId, int term, string examTypeName)
        {
            // 0. Fetch academic year for footer
            var academicYear = await _context.AcademicYears
                .FirstOrDefaultAsync(ay => ay.Id == academicYearId);
            ...
            if (grade == null)
                throw new ArgumentException($"Grade with ID {gradeId} not found or is not active");
            ...
            var document = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Content().Column(col =>
                    {
                        col.Item().Text($"Mark Schedule - {grade.Name} {grade.Stream} - Academic Year {academicYearId}, Term {term}, Exam: {examTypeName}").FontSize(16).Bold().AlignCenter();
                        col.Item().Height(20);
